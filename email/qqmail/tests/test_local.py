...
class MailTests(unittest.TestCase):
    config = {"account": {"email": "me@example.com", "password": "secret"}, "smtp": {"host": "localhost", "port": 465}, "imap": {"host": "localhost", "port": 993}}
    ...
    def test_missing_or_invalid_authorization_code_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "qqmail"
            (root / "config").mkdir(parents=True)
            (root / "SKILL.md").write_text("# skill", encoding="utf-8")
            script = root / "core" / "scripts" / "mail.py"
            with self.assertRaisesRegex(ValueError, "无法读取文件"):
                load_config(script)
            (root / "config" / "config.json").write_text('{"smtp": {}, "imap": {}, "account": {"email": "me@qq.com"}}', encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "客户端授权码"):
                load_config(script)
